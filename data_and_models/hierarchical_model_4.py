...
class model_fn:
    def __new__(cls, n_dim, n_samples) -> Tuple[Callable, Callable, int]:
        _model = tfd.JointDistributionNamed(dict(
            gamma_scale=tfd.HalfNormal(1.0),
            gamma_loc=tfd.Normal(0.0, 1.0),
            gamma=lambda gamma_loc, gamma_scale: tfd.Independent(
                tfd.Normal(
                    jnp.full(2, gamma_loc),
                    jnp.full(2, gamma_scale)
                ), 1),
            beta_scale=tfd.HalfNormal(1.0),
            beta=lambda gamma, beta_scale: tfd.Independent(
                tfd.Normal(
                    jnp.repeat(gamma, 5),
                    beta_scale,
                ), 1),
            y=lambda beta: tfd.Independent(tfd.Normal(beta, 1.0), 1),
        ))
...
def get_config():
    ...
    config.name = "hierarchical_model_4"
