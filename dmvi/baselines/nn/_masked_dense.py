...
class MaskedDense(hk.Linear):
    ...
    def __call__(self, inputs, *, precision: Optional[lax.Precision] = None) -> jnp.ndarray:
        if not inputs.shape:
        ...
        if self.with_bias:
            b = hk.get_parameter("b", (jnp.shape(self.mask)[-1],), dtype, init=self.b_init)
